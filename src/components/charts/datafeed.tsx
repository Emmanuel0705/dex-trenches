      ...
      try {
        ...
        const subId = client.streams.subscribe(
          ...
          (candle: any) => {
            ...
            const candleTime =
              typeof candle.time === "number" && candle.time > 10000000000
                ? candle.time / 1000
                : candle.time;
            ...
            if (!firstCandleHandled) {
              ...
              if (lastBarTime && lastBar && firstCandleTime > lastBarTime) {
                ...
                const updatedHigh = Math.max(
                  firstCandle.high ?? updatedOpen,
                  updatedOpen,
                );
                const updatedLow = Math.min(
                  firstCandle.low ?? updatedOpen,
                  updatedOpen,
                );
